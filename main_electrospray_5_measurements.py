...
def classify_sample(processing, result, ml_models, db):
    ...
    try:
        ...
        all_features.update({
            ...
            "deviation_ratio": float(result["deviation_na"]) / float(result["mean_na"])
        })
...
if __name__ == "__main__":
    ...
    print(f"[MAIN] Timing:       stab={stab_time}s  acquire≥0.5s x 5  "
    ...
    try:
        for flow_rate in meas["flow_rate"]:
            ...
            for voltage in steps:
                ...
                # Take 5 measurements per voltage step
                for measurement_idx in range(5):
                    ...
                    # The camera trigger should only happen on the 5th (last) measurement
                    ...
                    result = acquire_and_process(
                        ...
                        processing,
                        trigger_fn = current_trigger_fn  # ← synchronized trigger
                    )
                    ...
                    print(f"         Meas {measurement_idx+1}/5: RF={rf_result}  XGB={xgb_result}  "
